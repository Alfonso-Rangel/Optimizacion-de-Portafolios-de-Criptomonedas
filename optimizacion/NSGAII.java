...
public class NSGAII {
    ...
    private ArrayList<Individuo> initPobl(double[] rf) {
        ...
        // ===== EXTREMOS =====
        for (int i = 0; i < dim && pop.size() < popSize; i++) {
    ...
    private ArrayList<Individuo> generarHijos(double[] rf, int gen) {
        ...
        // ===== MUTACIÓN CONTROLADA =====
        double mutScale = 0.2 + 0.8 * (1.0 - (double) gen / maxGen);
    ...
    private void calcularCrowdingDistance(ArrayList<Individuo> frente) {
        ...
        for (int m = 0; m < 2; m++) {
            ...
            double range = frente.get(n - 1).obj[idx] - frente.get(0).obj[idx];

            if (range == 0) continue;

            for (int i = 1; i < n - 1; i++) {
                frente.get(i).crowdDist += (frente.get(i + 1).obj[idx] - frente.get(i - 1).obj[idx]) / range;
            }
        }
    }
